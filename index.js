...
function orderPlace(){
    ...
    if (!(/^[\sא-ת]+$/.test(name)))
        nameFlag = 0; // If the input is not valid, set the flag to 0 (false).
    ...
    // If both fields are valid, construct a message with the order details and display it using an alert box.
        ...
        let message = "ההזמנה הושלמה" + "<br>" +
            "שם מלא: " + name + "<br>" +
            "טלפון נייד: " + phoneNum + "<br>" +
            "כמות אנשים: " + numPeople + "<br>" +
            "תאריך: " + date + "<br>" +
            "שעה: " + time;
        alert("ההזמנה הושלמה" + "\n" + "שם מלא: " + name + "\n" + "טלפון נייד: " + phoneNum + "\n" + "כמות אנשים: " + numPeople + "\n" + "תאריך: " + date + "\n" + "שעה: " + time);
        // Display the order details in a HTML element with ID 'message'.
